...
public class TomatoClient extends JFrame implements ActionListener {
	...
	public static void main(String args[]) {
		...
		//화면 부륾
		tc.initDisplay();
		//소켓 생성 - TS쪽 ServerSocket감지 - 일반 소켓 전달됨. - run메소드 안에서 TomatoSerThread생성
		//생성자 호출(this) - 듣기 가능해짐. - 전제 조건 : oos, ois, 소켓 객체가 있어야 가능함.
		tc.init();
	}
	//소켓 관련 초기화
	public void init() {
		try {
			//서버측의 ip주소 작성하기
			socket = new Socket("192.168.219.102",3002);
			//홀수 소켓에 대한 처리
			...
			//서버에 말을 한 후 들을 준비를 한다. - 대기를 한다. - 듣기 - 프로토콜을 비교해야 한다.
			//프로토콜 설계하기 - ERD 그린다. 데이터 클래스 설계 - List, Map 단위 테스트 너가 해줄래
