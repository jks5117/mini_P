...
public class JComboBoxTest1 extends JFrame implements ItemListener {
	...
	public static void main(String[] args) {
		new JComboBoxTest1();//생성자를 경유하고 생성자 화면처리 메소드 호출해보기
	}
	...
	public void itemStateChanged(ItemEvent ie) {//콜백메소드이다. -시스템레벨에서 이벤트가 감지 되었을 때 호출됨.
		...
		if(obj == jcb_top) {
			if(ie.getStateChange() == ItemEvent.SELECTED) {
				...
				jcb_mid.removeAllItems();//기존의 totals에 있던 전체를 삭제하고 새로운 아이템으로 변경 준비
				if("가전".equals(top)) {
					//if(top.equals("가전"))
